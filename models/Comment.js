  ...
  try {
    if (userId != undefined) {
      const res = await pool.query(
        'SELECT c.id, dateTime, userid, postid, content, username, profilephoto, c.numoflikes, l.val \
        FROM comments c \
        INNER JOIN users u ON c.userid = u.id \
        INNER JOIN likescomment l ON l.postid = $1 AND l.commentid = c.commentid AND l.user = $2 WHERE postid = $1',
        [postId, userId]
      );
      return res.rows;
    }
...
//   const { votedId: commentId } = data.body;
//   try {
//     const res = await pool.query(
...
//   try {
//     const res = await pool.query(
//       'INSERT INTO likescomment (userid, commentid, val) VALUES ($1, $2, $3) RETURNING *',
//       [userId, commentId, 1]
//     );
//     return res.rows[0];
//   } catch (err) {
